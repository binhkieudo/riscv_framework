  ...
  extends SPIFlashXilinxPlacedOverlay(name, designInput, shellInput)
{
  ...
  shell { InModuleBody {
    val packagePinsWithPackageIOs = Seq(("C15", IOPin(io.qspi_sck)),
      ("E15", IOPin(io.qspi_cs)),
      ("E16", IOPin(io.qspi_dq(0))),
      ("D15", IOPin(io.qspi_dq(1))),
      ("K15", IOPin(io.qspi_dq(2))),
      ("J15", IOPin(io.qspi_dq(3))))
