...
class Arty100THarness(override implicit val p: Parameters) extends Arty100TShell {
  ...
  val other_leds = all_leds.drop(12).take(2)
  val status_leds = all_leds.drop(14).take(2)
  ...
  class HarnessLikeImpl extends Impl with HasHarnessInstantiators {
    ...
    def success = { require(false, "Error"); false.B }
    ...
    other_leds(1) := ddrOverlay.mig.module.io.port.init_calib_complete
