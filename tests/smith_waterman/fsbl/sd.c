...
int main(int mhartid, char** dump)
{
	...
	do {
		...
		core_id = (unsigned int)(*threadRdPtr)->core_id;
		if ((core_id == hart_id) || (core_id == 0xffffffff)) {
			if (threadRdPtr_val < threadWrPtr_val) {
				...
				*threadRdPtr = (volatile struct threadPointer*)threadRdPtr_val;
				mux_unlock();
				// Run thread
				void (*func_ptr)(int, unsigned int, unsigned int) = (void*)address;
				(*func_ptr)(mhartid, thread_id, core_id);
				// mux_unlock();
			}
